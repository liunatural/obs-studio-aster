...
static void OBSHxFlvStreamStarting(void *data, calldata_t *params)
{
	BasicOutputHandler *output = static_cast<BasicOutputHandler*>(data);
	obs_output_t *obj = (obs_output_t*)calldata_ptr(params, "output");
	...
	output->delayActive = true;
	QMetaObject::invokeMethod(output->main,
		"HxFlvStreamDelayStarting", Q_ARG(int, sec));
}

static void OBSHxFlvStreamStopping(void *data, calldata_t *params)
{
	BasicOutputHandler *output = static_cast<BasicOutputHandler*>(data);
	obs_output_t *obj = (obs_output_t*)calldata_ptr(params, "output");
	...
	int sec = (int)obs_output_get_active_delay(obj);
	if (sec == 0)
		QMetaObject::invokeMethod(output->main, "HxFlvStreamStopping");
	else
		QMetaObject::invokeMethod(output->main,
			"HxFlvStreamDelayStopping", Q_ARG(int, sec));
}

static void OBSHxFlvStartStreaming(void *data, calldata_t *params)
{
	BasicOutputHandler *output = static_cast<BasicOutputHandler*>(data);
	output->streamingActive = true;
	QMetaObject::invokeMethod(output->main, "HxFlvStreamingStart");

	UNUSED_PARAMETER(params);
}

static void OBSHxFlvStopStreaming(void *data, calldata_t *params)
{
	BasicOutputHandler *output = static_cast<BasicOutputHandler*>(data);
	int code = (int)calldata_int(params, "code");
	const char *last_error = calldata_string(params, "last_error");
	...
	output->delayActive = false;
	QMetaObject::invokeMethod(output->main,
		"HxFlvStreamingStop", Q_ARG(int, code), Q_ARG(QString, arg_last_error));
}
...
struct SimpleOutput : BasicOutputHandler {
	...
	virtual bool StartHxFlvStreaming() override;  //liuzhi
	virtual void StopHxFlvStreaming(bool force) override;//liuzhi
...
//liuzhi
bool SimpleOutput::StartHxFlvStreaming()
{
	if (!Active())
		SetupOutputs();

	const char *type = "hengxin_flv_output";
	...
	if (outputType != type) {
		...
		streamDelayStarting.Connect(
			obs_output_get_signal_handler(streamOutput),
			"starting", OBSHxFlvStreamStarting, this);
		streamStopping.Connect(
			obs_output_get_signal_handler(streamOutput),
			"stopping", OBSHxFlvStreamStopping, this);

		startStreaming.Connect(
			obs_output_get_signal_handler(streamOutput),
			"start", OBSHxFlvStartStreaming, this);
		stopStreaming.Connect(
			obs_output_get_signal_handler(streamOutput),
			"stop", OBSHxFlvStopStreaming, this);
...
//liuzhi
void SimpleOutput::StopHxFlvStreaming(bool force)
{
	if (force)
...
//liuzhi
bool AdvancedOutput::StartHxFlvStreaming()
{
	if (!useStreamEncoder ||
		(!ffmpegOutput && !obs_output_active(fileOutput))) {
		UpdateStreamSettings();
...
//liuzhi
void AdvancedOutput::StopHxFlvStreaming(bool force)
{
	if (force)
		obs_output_force_stop(streamOutput);
	else

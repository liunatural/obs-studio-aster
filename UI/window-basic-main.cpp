...
//liuzhi
void OBSBasic::ResetHxFlvOutputs()
{

	ProfileScope("OBSBasic::ResetHxFlvOutputs");

	const char *mode = config_get_string(basicConfig, "Output", "Mode");
	bool advOut = astrcmpi(mode, "Advanced") == 0;

	if (!hxFlvOutputHandler || !hxFlvOutputHandler->Active()) 
	{
		hxFlvOutputHandler.reset();
		hxFlvOutputHandler.reset(advOut ?
			CreateAdvancedOutputHandler(this) :
			CreateSimpleOutputHandler(this));
	}
	else
	{
		hxFlvOutputHandler->Update();
	}
}
...
//liuzhi
void OBSBasic::HxFlvStartStreaming()
{

	if (hxFlvOutputHandler->StreamingActive())
		return;
	if (disableOutputsRef)
		return;


	ui->hxFlvstreamButton->setEnabled(false);
	ui->hxFlvstreamButton->setText(QTStr("Basic.Main.HxFlvStreamStarting"));//显示正在输出FLV的状态

	if (sysTrayHxFlvStream) {
		sysTrayHxFlvStream->setEnabled(false);
		sysTrayHxFlvStream->setText(ui->hxFlvstreamButton->text());
	}

	if (!hxFlvOutputHandler->StartHxFlvStreaming()) {
		ui->hxFlvstreamButton->setText(QTStr("Basic.Main.StartHxFlvStreaming"));
		ui->hxFlvstreamButton->setEnabled(true);

		if (sysTrayHxFlvStream) {
			sysTrayHxFlvStream->setText(ui->hxFlvstreamButton->text());
			sysTrayHxFlvStream->setEnabled(true);
		}
...
void OBSBasic::HxFlvStopStreaming()
{
	SaveProject();

	if (hxFlvOutputHandler->StreamingActive())
		hxFlvOutputHandler->StopHxFlvStreaming(streamingStopping);
...
void OBSBasic::HxFlvStreamingStart()
{
	ui->hxFlvstreamButton->setText(QTStr("Basic.Main.StopHxFlvStreaming"));
	ui->hxFlvstreamButton->setEnabled(true);
	ui->statusbar->StreamStarted(outputHandler->streamOutput);

	if (sysTrayStream) {
		sysTrayStream->setText(ui->hxFlvstreamButton->text());
		sysTrayStream->setEnabled(true);
	}
...
void OBSBasic::HxFlvStreamingStop(int code, QString last_error)
{
	...
	if (!startStreamMenu.isNull()) {
		ui->hxFlvstreamButton->setMenu(nullptr);
		startStreamMenu->deleteLater();
...
void OBSBasic::HxFlvStreamDelayStarting(int sec)
{
	ui->hxFlvstreamButton->setText(QTStr("Basic.Main.StopHxFlvStreaming"));
	ui->hxFlvstreamButton->setEnabled(true);
	...
	startStreamMenu = new QMenu();
	startStreamMenu->addAction(QTStr("Basic.Main.StopHxFlvStreaming"),
		this, SLOT(StopStreaming()));
	startStreamMenu->addAction(QTStr("Basic.Main.ForceStopStreaming"),
		this, SLOT(ForceStopStreaming()));
	ui->hxFlvstreamButton->setMenu(startStreamMenu);
...
void OBSBasic::HxFlvStreamDelayStopping(int sec)
{
	ui->hxFlvstreamButton->setText(QTStr("Basic.Main.StartHxFlvStreaming"));
	ui->hxFlvstreamButton->setEnabled(true);
	...
	startStreamMenu = new QMenu();
	startStreamMenu->addAction(QTStr("Basic.Main.StartHxFlvStreaming"),
...
void OBSBasic::HxFlvStreamStopping()
{
	ui->hxFlvstreamButton->setText(QTStr("Basic.Main.HxFlvStreamStopping"));
...
//liuzhi
void OBSBasic::on_hxFlvstreamButton_clicked()
{
...
void OBSBasic::SystemTrayInit()
{
	...
	sysTrayStream = new QAction(QTStr("Basic.Main.StartStreaming"),
			trayIcon);
	sysTrayHxFlvStream = new QAction(QTStr("Basic.Main.StartHxFlvStreaming"),
		trayIcon);;//liuzhi
	...
	connect(sysTrayHxFlvStream, SIGNAL(triggered()),
		this, SLOT(on_hxFlvstreamButton_clicked()));//liuzhi
